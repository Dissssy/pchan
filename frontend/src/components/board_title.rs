...
pub fn BoardTitle() -> Html {
    ...
    let board: UseStateHandle<ApiState<BoardWithThreads>> = use_state(|| ApiState::Pending);
    {
        ...
        use_effect_with(board_ctx, move |board_ctx| {
    ...
    board.standard_html("BoardTitle", |board| {
        html! {
            <BoardName prefix={"board-title"} first={true} last={true} board={SafeBoard::from(board.clone())} view={BoardNameType::Both} hover={None} />
        }
    }).unwrap_or_else(|e| {
        match nav {
            Some(nav) => {

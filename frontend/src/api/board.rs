use super::ApiError;
use common::structs::{Banner, BoardWithThreads, SafeBoard};
...
pub async fn get_banner(token: &str, board: &str) -> Result<Banner, ApiError> {
    let res = Request::get(&format!("/api/v1/board/{}/banner", board))
        .header("authorization", token)
        .send()
        .await
        .map_err(|e| match e {
